 ...
 *
 * 왜 필요한가: 판당 LP 증감은 "한 게임을 사이에 둔 두 LP 측정값의 차이"로만 정확히 구할 수 있다.
 * 수동 갱신만으로는 측정이 드물어 한 구간에 여러 판이 섞이고, 그러면 어느 판 몫인지 쪼갤 수 없어 표시되지 않는다.
 * 게임 1판(~30분)보다 짧은 주기로 폴링하면 매 랭크 게임 사이에 측정값이 한 번씩 찍혀 단일 게임 구간이 생기고,
 * 그제야 판당 LP가 op.gg 처럼 채워진다.
 *
 * 실제 갱신 작업(매치 수집 + LP 갱신 + LP 측정값 적재)은 큐에 넣기만 하면 RefreshJobWorker 가 수행한다.
