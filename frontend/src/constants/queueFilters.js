...
export const QUEUE_TYPE_OPTIONS = [
  ...
  { key: 'URF',     label: 'U.R.F.',        queues: [900, 1010, 1900] },
  { key: 'CLASH',   label: '격전',          queues: [700, 720] },
  { key: 'ARENA',   label: '아레나',         queues: [1700, 1710] },
  { key: 'NEXUS',   label: '돌격! 넥서스',   queues: [1300] },
  { key: 'DOOM',    label: '초토화 봇',      queues: [950, 960] },
  { key: 'SPECIAL', label: '특별 게임 모드',  queues: [1200, 1400, 610, 1810, 1820, 1830, 1840] },
];

export const ALL_QUEUE_FILTERS = {
  ...QUEUE_FILTERS,
  ...Object.fromEntries(QUEUE_TYPE_OPTIONS.map(o => [o.key, { label: o.label, queues: o.queues }])),
};

export const DROPDOWN_FILTER_KEYS = new Set(QUEUE_TYPE_OPTIONS.map(o => o.key));

// 선택된 필터 key로 매치 목록 필터링
export function filterMatchesByQueue(matches, key) {
  const def = ALL_QUEUE_FILTERS[key];
  if (!def || def.queues == null) return matches;
  const set = new Set(def.queues);
  return matches.filter(m => set.has(m.queueId));
}
